...
export default function FilesExample() {
  ...
  async function handleCheck(product) {
    ...
    try {
      const url = "http://localhost:5001/api/customer/updateCart"
  ...
  async function handleAddSubscription(){
    try {
      const url = "http://localhost:5001/api/customer/addSub"
  ...
  React.useEffect(() => {
    const getData = async () => {
      try {
        const url = "http://localhost:5001/api/customer/getCart"
